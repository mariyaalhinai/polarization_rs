...
def load_data(filename):
    ...
    #                 pbc=True)
    # df['species'] = df['polar_structure'].map(lambda x: list(set(x.get_chemical_symbols())))
    df['structure'] = df['polar_structure']
    df['species'] = df['structure'].map(lambda x: x.species).apply(set)
    species = sorted(list(set(df['structure'].map(lambda x: x.species).sum())))
    # structure = sorted(list(set(df['structure'].sum())))
...
def train_valid_test_split(df, species, valid_size, test_size, seed=12, plot=False):
    ...
    # print(len(idx_train), len(idx_dev))
    ...
    # print(len(idx_valid), len(idx_test))
