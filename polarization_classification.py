...
# %%
data_file = '/home/mariyaah/data1/GNNOpt/classification/data_class.pkl'
...
def roman_to_int(roman: str) -> int:
    roman_map = {
        ...
        'IA': 1, 'IIA': 2, 'IIIA': 3, 'IVA': 4, 'VA': 5,
        'VIA': 6, 'VIIA': 7, 'VIIIA': 8, 'IB': 11, 'IIB': 12
    }
    return roman_map.get(roman, 0)  # default to 0 if unknown
...
for Z in tqdm(range(1, 119), bar_format=bar_format):  # Elements 1 to 118
    specie = Atom(Z)
    ...
    # Group number (handle Roman numeral or integer)
    if isinstance(e.group, str):
        Z_group = roman_to_int(e.group)
    elif isinstance(e.group, int):
        Z_group = e.group
    else:
        Z_group = 0
    ...
    Z_ox = max(e.oxistates) if e.oxistates else 0
    ...
    specie_group.append(Z_group)
    specie_en.append(Z_en)
    specie_ea.append(Z_ea)
    specie_ie.append(Z_ie)
    specie_oxidation.append(Z_ox)


# Convert selected features to torch diagonal matrices
...
group_onehot = torch.diag(torch.Tensor(specie_group))
en_onehot = torch.diag(torch.Tensor(specie_en))
ea_onehot = torch.diag(torch.Tensor(specie_ea))
ie_onehot = torch.diag(torch.Tensor(specie_ie))
oxidation_onehot = torch.diag(torch.Tensor(specie_oxidation))
...
def build_data(entry, type_encoding, type_onehot, r_max=5.):
    ...
    data = tg.data.Data(
        ...
        x_group=group_onehot[idx],
        x_en=en_onehot[idx],
        x_ea=ea_onehot[idx],
        x_ie=ie_onehot[idx],
        x_oxidation=oxidation_onehot[idx],
        # Edge features
        edge_index=torch.stack([torch.LongTensor(edge_src), torch.LongTensor(edge_dst)], dim=0),
        edge_shift=torch.tensor(edge_shift),
        edge_vec=edge_vec,
        edge_len=edge_len,
        y= torch.tensor([1 if entry.polarization > threshold else 0], dtype=torch.long)
    )
...
import os
os.makedirs('model/mariya', exist_ok=True)
...
class PeriodicNetwork(Network):
    ...
    # override the `reduce_output` keyword to instead perform an average over atom contributions    
        ...
        self.em_group = nn.Linear(in_dim, em_dim)
        self.em_en = nn.Linear(in_dim, em_dim)
        self.em_ea = nn.Linear(in_dim, em_dim)
        self.em_ie = nn.Linear(in_dim, em_dim)
        self.em_oxidation = nn.Linear(in_dim, em_dim)

        self.em_mixing = MixingLinear(14, 1)
...
import torch.nn.functional as F
run_name = 'model_classification_' + run_time

opt = torch.optim.AdamW(model.parameters(), lr=0.005, weight_decay=0.05)
scheduler = torch.optim.lr_scheduler.ExponentialLR(opt, gamma=0.96)

loss_fn = torch.nn.CrossEntropyLoss()
loss_fn_mae = torch.nn.CrossEntropyLoss()
...
output_dir = "/home/mariyaah/data1/GNNOpt/classification/model_output"
os.makedirs(output_dir, exist_ok=True)
path = lambda name: os.path.join(output_dir, name)

np.savetxt(path(run_name + '_CE_loss.txt'), np.column_stack((steps, loss_train, loss_valid)), fmt='%.8f', delimiter='\t')
...
df['CE'] = np.empty((len(df),), dtype=int)
df['y_pred'] = df['polarization'].map(lambda p: 1 if p > threshold else 0)
...
with torch.no_grad():
    ...
    for i, d in tqdm(enumerate(dataloader), total=len(dataloader), bar_format=bar_format):
        d.to(device)
        output = model(d)
        loss = F.cross_entropy(output, d.y, reduction='none').cpu().numpy()
        df.loc[i0:i0 + len(d.y) - 1, 'y_pred'] = [[k] for k in output.cpu().numpy()]
        df.loc[i0:i0 + len(d.y) - 1, 'CE'] = loss
        i0 += len(d.y)

df['y_pred'] = df['y_pred'].map(lambda x: x[0])

preds = np.array([int(x[0]) if isinstance(x, (np.ndarray, list)) else int(x) for x in df['y_pred'].values])
np.savetxt(path(run_name + '_y_pred.txt'), preds, fmt='%i', delimiter='\t')

df['true_class'] = df['polarization'].map(lambda p: 1 if p > threshold else 0)
df_valid = df.iloc[idx_valid]
df_train = df.iloc[idx_train]

true_vals = np.array([int(x[0]) if isinstance(x, (list, np.ndarray)) else int(x) for x in df_valid["true_class"].values])
pred_vals = np.array([int(x[0]) if isinstance(x, (list, np.ndarray)) else int(x) for x in df_valid["y_pred"].values])

true_vals_train = np.array([int(x[0]) if isinstance(x, (list, np.ndarray)) else int(x) for x in df_train["true_class"].values])
pred_vals_train = np.array([int(x[0]) if isinstance(x, (list, np.ndarray)) else int(x) for x in df_train["y_pred"].values])

np.savetxt(
    path(run_name + '_true_vs_pred_valid.txt'),
    np.column_stack((true_vals, pred_vals)),
    fmt='%i\t%i',
    header='True_Class\tPredicted'
)

np.savetxt(
    path(run_name + '_true_vs_pred_train.txt'),
    np.column_stack((true_vals_train, pred_vals_train)),
    fmt='%i\t%i',
    header='True_Class\tPredicted'
)
...
disp.plot()
plt.tight_layout()
plt.savefig(path(run_name + '_confusion_matrix_valid.pdf'))

cm = confusion_matrix(true_vals_train, pred_vals_train)
disp = ConfusionMatrixDisplay(confusion_matrix=cm)
disp.plot()
plt.tight_layout()
plt.savefig(path(run_name + '_confusion_matrix_train.pdf'))

print(f"Accuracy (Valid) = {accuracy_score(true_vals, pred_vals):.4f}")
print(f"Accuracy (Train) = {accuracy_score(true_vals_train, pred_vals_train):.4f}")
print(classification_report(true_vals, pred_vals))
print(classification_report(true_vals_train, pred_vals_train))
